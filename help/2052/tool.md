...
#全局快捷键
每一个自定义工具都可以分配一个全局快捷键。当按下该快捷键时无论当前语法文件是什么，都会触发该工具。如需设定，请到快捷键中检索该工具的标题。

#分组快捷键
有时候我们需要按下同一个快捷键在不同的语法文件下调用不同的工具来实现诸如编译等功能。在EverEdit中定义了cm_tool_tool1~9这9个快捷键，指示了当前分组所包含的9个工具。您可以在快捷键设置中进行自定义。
